 ...
 */
export function createScriptedInputSender(transport: Transport): (sample: InputSample) => void {
  let seq = 0;
  ...
  return (sample: InputSample) => {
    ...
    transport.send(encodeMessage({ type: MessageType.InputBatch, firstSeq, frames: history.slice() }));
    seq++;
  };
}
